...
public class QuestionAudioService
{
    ...
    public QuestionAudioService(HttpClient httpClient, QuizAuthStateProvider authStateProvider)
    {
        _httpClient = httpClient;
        _authStateProvider = authStateProvider;
    }

    public async Task<QuizApiResponse> UploadQuestionAudioAsync(int questionId, IBrowserFile audioFile)
    {
        try
        {
            ...
            {
                return QuizApiResponse.Failure("Only admins and teachers can upload question audio");
            }
            ...
            var response = await _httpClient.PostAsync($"/api/question-audios/upload/{questionId}", formData);
    ...
    public async Task<QuizApiResponse> RemoveQuestionAudioAsync(int questionId)
    {
        try
        {
            // Check if user is authenticated and has the required role
            ...
            if (user == null ||
                !(user.Role == UserRole.Admin.ToString() || user.Role == UserRole.Teacher.ToString()))
            {
                return QuizApiResponse.Failure("Only admins and teachers can remove question audio");
            }
            ...
            var response = await _httpClient.DeleteAsync($"/api/question-audios/remove/{questionId}");
